...
class Contenedor{
    ...
    save = async (objetoAGuardar)=>{
        ...
        await fs.promises.writeFile(`./Desafios/${this.nombreArchivo}`,JSON.stringify(productsParsed),(err)=>{
        if (err){
        console.log("ERROR")
        } else {
    ...
    getAll = async () => {
        try {
            const products = await fs.promises.readFile(`./Desafios/${this.nombreArchivo}`,"utf-8");
            let productsParsed;
            return productsParsed = JSON.parse(products);
        } catch (error) {
            await fs.promises.writeFile(`./Desafios/${this.nombreArchivo}`, JSON.stringify([]));
		    products = [];
        }
    ...
    deleteById= async (id) =>{
        try {
            let productsParsed = await this.getAll();
            console.log(productsParsed)
            let arrayConProductoEliminado = productsParsed.filter((element) => element.id != id);
